...
class NaviTest(unittest.TestCase):
    ...
    def test_direct_to_view_calendar(self):
        driver = self.driver
        driver.find_element_by_id("sbViewCalendars").click()
        header = driver.find_element_by_id("schedulesTitle")
        self.assertIn("Select the trimester calendar that you would like view", header.text)
    ...
    def test_direct_to_view_appeals(self):
        ...
        header[0].click()
        header = driver.find_element_by_tag_name("h5")
        self.assertIn("Appeal Results", header.text)
