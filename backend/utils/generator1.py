...
def readJson():
    ...
    for key,value in wjson.items():
        ...
        if courses[i].courseName=="01.400":                 #check if it is capstone
            ...
            newCourse.append(i)
        i+=1
    return courses,newCourse,CapstoneNum
...
def initializeValue(courses):                      #initialize rooms, cohort list, professor list
...
def checkAllSchedule(rowRef,schedule,classList,profList,roomList,i,j):# check availability for class, profs and rooms. return the room number if everything is satisfied, otherwise return -1
...
def setAllSchedule(rowRef,schedule,classList,profList,room,i,j,value):     #set the schedule of all classes, profs and room involved to be certain value
    if type(classList)==list:
        for classes in classList:
...
def capstoneInitial(courses,schedule,cap):                                              # initialize the capstion schedule with courseID cap (capstone)
    global claNum
...
def generator(courses,rowRef,CapstoneNum,c1,c2,c3,c4,b1,b2,seniorCourse):                                  # main generator function, takes in 4 params for priority, 2 params for lunch break and 1 term678 course list
    #global CapstoneNum
    ...
    if CapstoneNum!=-1:
        newSche = capstoneInitial(courses,newSche,CapstoneNum)
    Classlist=newClassList(courses,seniorCourse)                                        # group the first class of every three term678 courses together.
                    ...
                    while (True):
                        ...
                        for newtime in range(dayTime[randomDay][0],dayTime[randomDay][1]-clength+2):
                            ...
                            if roomID==-1:
                                ...
                                newSche=setAllSchedule(rowRef,newSche,randomClass,courses[count].sessions[l]["proID"],roomID,newtime,newtime+clength,value) #block the timing
...
def checkScore(courses,rawSchedule):
...
def formatOutput(courses,rowRef,CapstoneNum,schedule):
    #global courses
...
"""
def mainFunction():
    courses,seniorCourse,CapstoneNum = readJson()                                               # read json file
    rowRef=referenceRows()                                                          # initialize row reference table
    courses,rowRef=initializeValue(courses)
    ...
    for i in range(90):                                                             # generate 90 schedules in the first round with random parameters
        while(True):
            ...
            currentSchedule=generator(courses,rowRef,CapstoneNum,param1,param2,param3,param4,randBreak1,randBreak2,seniorCourse)
    ...
    currentTime=lastTime
    index,score=checkScore(courses,rawSchedules)
    ...
    for i in range(5):                                                              # generate 81 schedules for the next 5 rounds, each time use the previously generated param list
        ...
        for j in range(81):
            while(True):
                currentSchedule=generator(courses,rowRef,CapstoneNum,rawParams[j][0],rawParams[j][1],rawParams[j][2],rawParams[j][3],rawParams[j][4],rawParams[j][5],rawParams[j][6])
                if currentSchedule[0][0]!="Fail":
                    lastTime=currentTime
        ...
        index,score=checkScore(courses,newlist)
    ...
    formatOutput(courses,rowRef,CapstoneNum,rawSchedule[index[0]])                                             # output the schedule in json format
